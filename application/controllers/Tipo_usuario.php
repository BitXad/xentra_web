...
class Tipo_usuario extends CI_Controller{
     ...
     */
    function edit($tipo_usuario)
    {   
        // check if the tipo_usuario exists before trying to edit it
        $data['tipo_usuario'] = $this->Tipo_usuario_model->get_tipo_usuario($tipo_usuario);
        
        if(isset($data['tipo_usuario']['tipo_usuario']))
        {
            if(isset($_POST) && count($_POST) > 0)     
            {   
                $params = array(
                ...
                $this->Tipo_usuario_model->update_tipo_usuario($tipo_usuario,$params);            
                redirect('tipo_usuario/index');
            }
            else
            {
     ...
     */
    function remove($tipo_usuario)
    {
        $tipo_usuario = $this->Tipo_usuario_model->get_tipo_usuario($tipo_usuario);

        // check if the tipo_usuario exists before trying to delete it
        if(isset($tipo_usuario['tipo_usuario']))
        {
            $this->Tipo_usuario_model->delete_tipo_usuario($tipo_usuario);
            redirect('tipo_usuario/index');
        }
        else
